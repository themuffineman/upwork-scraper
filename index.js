...
server.post('/scrape/yelp', async (req, res) =>{
  ...
  try {
    ...
    await page.goto(`https://www.yelp.com/search?find_desc=${industry}&find_loc=${location}&start=${pagination*10}`)
    ...
    const results = await page.$$eval("ul.list__09f24__ynIEd li", ()=>{
      ...
      for(const item of list){
        ...
        const businessWebsite = await businessPage.$eval('a.y-css-1rq499d', (el) => getRootDomain(el.href, browser) )
        const businessPhone = await businessPage.$eval('p.y-css-1o34y7f', (el) => el.innerText)
        
        result.push({name, website: businessWebsite, phone: businessPhone, source: 'yelp'})
